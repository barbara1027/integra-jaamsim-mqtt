...
import com.jaamsim.Mqtt.MqttEntity;
...
public class MqttSend extends LinkedComponent {
	...
	public void addEntity(DisplayEntity ent) {
		try {
			...
			MqttEntity message = new MqttEntity();
			
			byte[] payload = message.getValue(getSimTime()).toString().getBytes();

			// 2. Cria a mensagem no formato que a biblioteca Paho entende, usando os bytes do payload.
			org.eclipse.paho.client.mqttv3.MqttMessage pahoMessage = new org.eclipse.paho.client.mqttv3.MqttMessage(payload);

			// 3. Publica a mensagem no formato correto.
			mqtt.publish(topic.getValue(), pahoMessage);
	...
	private Object generatePayload(DisplayEntity ent) throws Exception {
		ExpResult result = ExpEvaluator.evaluateExpression(expression.getValue(), ent,  getSimTime());
