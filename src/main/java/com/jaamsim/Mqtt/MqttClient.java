...
public class MqttClient extends DisplayEntity implements MqttCallback {
	...
	public void earlyInit() {
		try {
			...
			String userNameValue = getValue(userName, null);
			...
			String clientIdValue = getValue(this.clientId, org.eclipse.paho.client.mqttv3.MqttClient.generateClientId());
        	String serverUriValue = serverUri.getValue();

			System.out.println(">>> [DEBUG] Tentando conectar a: " + serverUriValue + " com ID de cliente: " + clientIdValue);

			client = new org.eclipse.paho.client.mqttv3.MqttClient(serverUri.getValue(), clientId);
			client.connect(options);
			client.setCallback(this);

			System.out.println(">>> [DEBUG] MqttClient: Conexão com o broker estabelecida com SUCESSO!");
		
		} catch (MqttException e) {
			//error("The MQTT client could not connect (reason code = " + e.getReasonCode() + ")");
			System.err.println("!!! [ERRO] MqttClient: Falha de Exception geral ao tentar conectar.");
			e.printStackTrace();
		} catch (Exception e) {
